...
class Workflow:
    ...
    # extras
    def visualize(
        self: WorkflowType, format: str = "pdf", view: bool = True, is_test: bool = False
    ) -> graphviz.Digraph:
        """
    ...
    def write_yml_to_file(self: WorkflowType, filename: Optional[str] = None) -> None:
        """Leverages the existing self.to_yml() function and writes the file to the default
           folder. From there, any CI/CD pipeline can pick up the new/updated workflow
           yml files. The suggested an default behavior is to create a folder in the root
           called `workflow-yamls` and store all workflow files there.

        Parameters
        ----------
            filename (Optional[str], optional): File path to store argo workflow yml files.".
        """
        if filename is None:
            folder = "workflow-yamls"
            # check if folder exists or not
            if not os.path.isdir(folder):
                # create folder
                Path(folder).mkdir(parents=True, exist_ok=True)

            filename = f"{folder}/{self.name}.yml"
